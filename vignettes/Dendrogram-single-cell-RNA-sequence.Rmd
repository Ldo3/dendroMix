...
sce_mat <- if (nrow(sce) < ncol(sce)) t(sce) else sce
N <- nrow(sce_mat)
...
# --- PCA to 10 components and grab first two for plotting ---
set.seed(0)
pca_res <- prcomp(sce_mat, center = TRUE, scale. = TRUE, rank. = 10)
pc2 <- as.data.frame(pca_res$x[, 1:2, drop = FALSE])
names(pc2) <- c("PC1","PC2")
pc2$cell_type <- cell_type

#Extract the transformed data
lowdim_data <- pca_res$x
dim(lowdim_data)  
...
p2 <- ggplot(df2, aes(k, height)) +
  ...
  labs(
    ...
    title = "Heights between levels in dendrogram"
  ) +
  theme_bw() +
  theme(plot.title.position = "plot",
        plot.title = element_text(margin = margin(b = 10), vjust = -0.2))
